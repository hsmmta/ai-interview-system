...
<head>
    ...
    <style>
        ...
        input[type="text"], input[type="email"], input[type="tel"] { width: 100%; padding: 12px; border: 1px solid #ddd; border-radius: 6px; box-sizing: border-box; font-size: 14px; transition: border-color 0.3s; }
...
<script>
    ...
    async function doRegister() {
        ...
        if(!phone || !code || !name) return showMsg('请填写必要信息');

        try {
            const res = await fetch('api/register', {
                method: 'POST',
                headers: {'Content-Type': 'application/json'},
                body: JSON.stringify({ action: 'register', phone, code, name, email, targetPosition })
            });
            const data = await res.json();
            if(data.success) {
