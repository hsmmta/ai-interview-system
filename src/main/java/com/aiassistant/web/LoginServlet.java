...
public class LoginServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        try {
            ...
            if ("sendCode".equals(action)) {
                ...
                if (user == null) {
                    writeError(resp, HttpServletResponse.SC_BAD_REQUEST, "未注册用户，请先注册");
                    return;
                }
            ...
            if ("verify".equals(action)) {
                String loginType = body.path("loginType").asText("code");
                if ("password".equals(loginType)) {
                    String password = body.path("password").asText("").trim();
                    handlePasswordLogin(req, resp, phone, password);
                } else {
    ...
    private void handlePasswordLogin(HttpServletRequest req, HttpServletResponse resp, String phone, String password) throws Exception {
        if (password.isEmpty()) {
            writeError(resp, HttpServletResponse.SC_BAD_REQUEST, "密码不能为空");
            return;
        }

        User user = UserService.findByPhone(phone);
        if (user == null) {
            writeError(resp, HttpServletResponse.SC_UNAUTHORIZED, "未注册用户，请先注册");
            return;
        }

        if (user.getPassword() == null || !user.getPassword().equals(password)) {
            writeError(resp, HttpServletResponse.SC_UNAUTHORIZED, "密码错误");
            return;
        }
