...
public class UserService {
    ...
    public static User registerUser(String phone, String name, String email, String password) throws Exception {
        ...
        user.setTargetPosition(""); // Initially empty
        user.setResumePath(""); // Initially empty
        createUser(user);
    ...
    public static void updateUser(User user) throws Exception {
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement ps = conn.prepareStatement(
                     "UPDATE users SET name = ?, email = ?, target_position = ?, nickname = ?, resume_path = ?, password = ? WHERE id = ?")) {
            ...
            ps.setString(5, user.getResumePath() != null ? user.getResumePath() : "");
    ...
    public static User findByPhone(String phone) throws Exception {
        try (Connection conn = DBUtil.getConnection();
            ...
            if (rs.next()) {
                ...
                user.setResumePath(rs.getString("resume_path")); // Read resume_path
    ...
    private static void createUser(User user) throws Exception {
        try (Connection conn = DBUtil.getConnection();
             PreparedStatement ps = conn.prepareStatement(
                     "INSERT INTO users (phone, nickname, created_at, last_login_at, name, email, target_position, resume_path, password) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?)")) {
            ...
            ps.setString(8, user.getResumePath());
